...
<?php
if ($submitName == 'Save')
    shn_form_hidden(array('id' => $staffTypeId));
else
    $volStatusName = "";
?>

<fieldset>
    <legend><?php echo ($submitName == 'Add') ? "Add" : "Edit"; ?> Type</legend>
            <input type="text" name ="org_name" size="45" class="required field text" value="<?php echo $staffTypeName ?>"/>

            <?php shn_form_submit($submitName, "class=\"styleTehButton\""); ?>
            <?php if ($conf['sems_enable']): ?>
                &nbsp&nbsp or &nbsp&nbsp
                <a class="styleTehButton" href="index.php?mod=srs&act=import_rmp_stafftypes">Import From Sahana RMP</a>
            <?php endif; ?>
</fieldset>
...
<table class="mainTable" style="width: 650px !important;">
    ...
    <tr class="head" height="30px">
        ...
        <?php if (is_array($staffTypes)) : ?>
            <?php $i=0;?>
            <?php foreach ($staffTypes as $row): ?>
                ...
                <tr>
                    ...
                    <td class="<?php echo $class; ?>">
                        <a class="" href="index.php?mod=srs&amp;act=staff_type_edit&amp;id=<?php echo $row['id'] ?>">Edit</a>
                    </td>
                    <td class="<?php echo $class; ?>">
                        <a class="confirmLink" href="index.php?mod=srs&amp;act=staff_type_del&amp;id=<?php echo $row['id'] ?>">Delete</a>
                    </td>
                </tr>
                <?php $i++; ?>
            <?php endforeach; ?>
